  ...
  ajax("GET", "item", function(items) {
    ...
    for (let key of items.content) {
        ...
        <div class="item linkblock"  id="${key.id}">
           <div class="hreff" cursor="pointer"> <div class="item-like"><button class="${key.like?"like-button-active":"item-like-button"}"></button>
            </div>
    ...
    //   [].forEach.call(document.querySelectorAll('.hreff'), function(el) {
        //   console.log(el);
        // el.addEventListener })
        ...
        for (let buttonItem of buttonItems) {
          buttonItem.addEventListener('click', function (event) {
            ('click', ajax("GET", "item/:itemId", function(itemCart) {
                console.log("+++");
